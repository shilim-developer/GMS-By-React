...
import {addEquipment} from '../../api/equipmentManageApi'
class EquipmentAddictionPage extends React.Component {
  constructor(props) {
    super(props)
    this.state = {equipmentTypeList: []}
  }
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        equipmentVo = Object.assign(equipmentVo, values)
        addEquipment(qs.stringify({equipment: equipmentVo.voToJson()}))
          .then((res) => {
            switch (res.data.serviceResult) {
              case 1:
      ...
      <QueueAnim duration="1200">
        <div key="equipmentAddiction" style={{padding: 15, background: '#fff', minHeight: 360}}>
          <Breadcrumb>
            ...
            <Breadcrumb.Item>新增器材</Breadcrumb.Item>
          </Breadcrumb>
          ...
          <Form>
            ...
            <FormItem {...formItemLayout} label="器材类型" hasFeedback>
              {getFieldDecorator('equipmentType.typeid', {
                ...
                ], initialValue: this.state.equipmentTypeList.length > 0 ? this.state.equipmentTypeList[0].typeid + '' : ''
              })(
                <Select>
...
EquipmentAddictionPage = Form.create()(EquipmentAddictionPage);

export default EquipmentAddictionPage
